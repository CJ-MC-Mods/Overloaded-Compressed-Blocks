...
public class CompressedBlockHandler {
  public static List<BlockCompressed> initFromConfig() {
    ...
    for (CompressedEntry entry : CompressedConfig.getCompressedEntries()) {
      List<Map.Entry<RegistryKey<Block>, Block>> matchedEntries = ForgeRegistries.BLOCKS.getEntries().stream().filter(e ->  e.getKey().getRegistryName().toString().matches(entry
          .baseRegistryName))
          .collect(Collectors.toList());

      if (matchedEntries.isEmpty()) {
        ResourceLocation rl = ResourceLocation.tryCreate(entry.baseRegistryName);
        createCompression(compressedBlocks, entry, rl.getPath(), null);
      } else {
        for (Map.Entry<RegistryKey<Block>, Block> matchedEntry : matchedEntries) {
          Block unCompressed = matchedEntry.getValue();
          createCompression(compressedBlocks, entry, matchedEntry.getKey().getRegistryName().getPath(), unCompressed);
        }
      }
  ...
  private static void createCompression(List<BlockCompressed> compressedBlocks, CompressedEntry entry, String entryName, Block unCompressed) {
    for (int i = 0; i < entry.depth; i++) {
      BlockCompressed compressedBlock = new BlockCompressed("compressed_" + entryName,
          unCompressed, entry, i + 1);
